  ...
  return (
    <div ref={ref}>
      <motion.div
        ...
        initial="hidden"
        animate="show"
        style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '60px 56px 10px', maxWidth: 1280, margin: '0 auto' }}
      >
        ...
        <motion.h1
          variants={fadeUp}
          style={{ fontFamily: 'Inter', fontWeight: 800, fontSize: 'clamp(80px, 10vw, 156px)', lineHeight: 0.92, letterSpacing: '-.04em', margin: 0, color: '#fff' }}
        >
          {t('hero.h1a')}<br />
        ...
        <motion.div
          variants={fadeUp}
          style={{ maxWidth: 560, margin: '28px auto 0', fontSize: 17, lineHeight: 1.6, color: 'rgba(245,238,226,.7)' }}
        >
          {t('hero.subtitle')}
        </motion.div>
        ...
        <motion.div
          variants={fadeUp}
          style={{ display: 'flex', gap: 12, marginTop: 30, justifyContent: 'center' }}
        >
          <motion.button
            whileHover={{ scale: 1.04, y: -2 }}
            whileTap={{ scale: 0.97 }}
            style={{ background: P.gold, color: P.night, border: 'none', padding: '15px 26px', fontSize: 12, letterSpacing: '.22em', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', borderRadius: 999 }}
          >
            {t('hero.appstore')}
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.04, y: -2, borderColor: 'rgba(245,238,226,.6)' }}
            whileTap={{ scale: 0.97 }}
            style={{ background: 'transparent', color: '#fff', border: '1px solid rgba(245,238,226,.3)', padding: '15px 26px', fontSize: 12, letterSpacing: '.22em', textTransform: 'uppercase', fontWeight: 600, cursor: 'pointer', borderRadius: 999 }}
          >
            {t('hero.googleplay')}
          </motion.button>
        </motion.div>
...
function Chapter({ ck, screen, i }) {
  ...
  return (
    <motion.div
      ...
      variants={staggerParent}
      style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'center', gap: 40, padding: '70px 100px', maxWidth: 1320, margin: '0 auto', direction: i % 2 === 1 ? 'rtl' : 'ltr' }}
    >
      <div style={{ direction: 'ltr' }}>
        ...
        <motion.h3
          variants={fadeUp}
          style={{ fontFamily: 'Inter', fontWeight: 800, fontSize: 64, lineHeight: 0.98, letterSpacing: '-.03em', margin: 0, color: '#fff' }}
        >
          {t(`${ck}.title`)}
        </motion.h3>
        <motion.p
          variants={fadeUp}
          style={{ maxWidth: 430, marginTop: 18, fontSize: 15, lineHeight: 1.7, color: 'rgba(245,238,226,.7)' }}
        >
          {t(`${ck}.desc`)}
        </motion.p>
        <motion.div
      ...
      <motion.div
        variants={{
          hidden: { opacity: 0, y: 60, rotate: i % 2 === 0 ? 4 : -4 },
...
function FinalCTA() {
  ...
  return (
    <motion.div
      ...
      style={{ position: 'relative', zIndex: 2, margin: '20px 56px 40px', padding: '60px 40px', background: P.card, border: `1px solid ${P.stroke}`, borderRadius: 20, textAlign: 'center', overflow: 'hidden' }}
    >
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <h3 style={{ fontFamily: 'Inter', fontWeight: 800, fontSize: 64, letterSpacing: '-.03em', margin: '14px 0 6px', color: '#fff' }}>
          <span style={{ fontWeight: 300, fontStyle: 'italic', fontFamily: 'Fraunces', color: P.gold }}>{t('cta.h2a')}</span>{' '}{t('cta.h2b')}
        </h3>
        <div style={{ color: P.muted, fontSize: 14 }}>{t('cta.sub')}</div>
        <div style={{ display: 'flex', gap: 12, marginTop: 28, justifyContent: 'center' }}>
          <motion.button
            whileHover={{ scale: 1.04, y: -2 }}
            whileTap={{ scale: 0.97 }}
            style={{ background: P.gold, color: P.night, border: 'none', padding: '16px 28px', fontSize: 12, letterSpacing: '.22em', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', borderRadius: 999 }}
          >
            {t('hero.appstore')}
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.04, y: -2 }}
            whileTap={{ scale: 0.97 }}
            style={{ background: '#fff', color: P.night, border: 'none', padding: '16px 28px', fontSize: 12, letterSpacing: '.22em', textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', borderRadius: 999 }}
          >
            {t('hero.googleplay')}
          </motion.button>
        </div>
  ...
  return (
    <section style={{ minHeight: '100vh', background: P.night, color: P.text, fontFamily: "'Inter', system-ui", position: 'relative', overflow: 'hidden' }}>
      <motion.nav
        ...
        style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', padding: '24px 56px', alignItems: 'center', borderBottom: `1px solid ${P.stroke}`, position: 'sticky', top: 0, zIndex: 50, background: 'rgba(11,21,51,.85)', backdropFilter: 'blur(12px)' }}
      >
        <div />
        <div style={{ textAlign: 'center', display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'center' }}>
          <img src={LOGO} alt="Sûkun" style={{ width: 28, height: 28, borderRadius: 6 }} />
          <span style={{ fontSize: 18, letterSpacing: '.35em', fontWeight: 700, color: P.gold }}>SÛKUN</span>
        </div>
        <div style={{ textAlign: 'right', display: 'flex', gap: 24, justifyContent: 'flex-end', fontSize: 11, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(245,238,226,.7)', fontWeight: 500 }}>
